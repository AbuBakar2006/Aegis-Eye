...
- Plain Python + pip project. No Anaconda, no notebooks. Run with: python aegiseye/main.py

## ARCHITECTURE — CRITICAL, DO NOT BREAK
aegiseye/core/detector.py implements a 3-thread "broadcast delay" pipeline:
1. READER thread: reads all video frames into a process queue at full speed
2. INFERENCE thread: runs Model A every FRAME_SKIP frames (default 15) at NATIVE 640 resolution, runs Model B only on accident frames, draws bounding boxes onto frames, pushes annotated frames to display queue. Maintains a rolling 10-sec frame buffer (aegiseye/core/buffer.py). On accident (conf > 0.5, outside 30s frame-based cooldown): computes severity (aegiseye/core/severity.py), saves blackbox clip (aegiseye/core/blackbox.py), sends alert (aegiseye/services/alert.py), generates PDF (aegiseye/services/report.py)
...
8. The tkinter popup in aegiseye/main.py is a temporary rough UI. Do not invest in polishing it — real UI/UX effort goes into the web dashboard (F7).
9. Record every change in CHANGELOG.md per the template.

## FILE MAP
- aegiseye/main.py — entry point, tkinter video selector + settings popup
- aegiseye/config.py — all constants (thresholds, cooldown, delay, paths, Twilio creds)
- aegiseye/core/detector.py — 3-thread pipeline (THE HEART — modify with extreme care)
- aegiseye/core/buffer.py — rolling deque frame buffer
- aegiseye/core/severity.py — F3: weighted optical flow severity (spike 40%, confidence 30%, magnitude 20%, vehicles 10%; >=0.60 High, >=0.35 Medium)
- aegiseye/core/blackbox.py — F4: FFmpeg H.264 clip export via imageio-ffmpeg
- aegiseye/services/alert.py — F5: Twilio SMS (currently TWILIO_ENABLED=False, simulated)
- aegiseye/services/report.py — F6: fpdf2 PDF with hyperlinked clip
- aegiseye/api/server.py — F7: FastAPI skeleton (endpoints: /api/incidents, clip/report downloads)
- aegiseye/export_onnx.py — one-time .pt → .onnx conversion
- test_videos/ — YouTube test videos (NOT training data)
- Storage/BlackBox-Clips/, Storage/Reports/ — auto-generated outputs
...
Run: python aegiseye/main.py → select a test video → confirm: (a) window shows smooth video with green/red/yellow boxes, (b) on accident: terminal prints confidence + vehicles + severity breakdown, (c) exactly ONE new MP4 in Storage/BlackBox-Clips/ and ONE new PDF in Storage/Reports/ per accident event, (d) PDF opens with a working clip hyperlink. If any of these fail after your change, revert.
